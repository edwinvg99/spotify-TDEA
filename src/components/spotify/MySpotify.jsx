  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Procesar playlists
        const playlistsData = playlistsRes.status === 'fulfilled' ? 
          await playlistsRes.value.json() : { total: 0 };

        // Procesar artistas
        const artistsData = artistsRes.status === 'fulfilled' ? 
          await artistsRes.value.json() : { items: [] };

        // Procesar géneros
        const genres = artistsData.items?.reduce((acc, artist) => {
    ...
    <div className="min-h-screen bg-gray-900 p-8 pb-24">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-12">
        ...
        {stats.topGenres.length > 0 && (
          <div className="bg-gray-800 rounded-lg p-6 hover:bg-gray-700 transition-colors">
            <h2 className="text-xl font-bold text-white mb-4">Géneros favoritos</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        {stats.topArtists.length > 0 && (
          <div className="bg-gray-800 rounded-lg p-6">
            <h2 className="text-xl font-bold text-white mb-4">Artistas más escuchados</h2>
        ...
        {stats.recentTracks.length > 0 && (
          <div className="bg-gray-800 rounded-lg p-6 hover:bg-gray-700/50 transition-all">
            <h2 className="text-xl font-bold text-white mb-4">Historial de reproducción</h2>
            <div className="space-y-4">
              {stats.recentTracks.slice(0, 5).map(({track, played_at}, index) => {
                // Formatear la fecha de manera segura
                const formattedTime = played_at ? new Date(played_at).toLocaleTimeString('es-ES', {
                  hour: '2-digit',
                  minute: '2-digit',
                  hour12: false
                }) : '';
