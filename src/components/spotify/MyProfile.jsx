...
import { auth, db, storage } from "../auth/AuthApp/firebase";
// Importaciones correctas por servicio
import { doc, getDoc, updateDoc, setDoc } from "firebase/firestore";
import { ref, uploadBytes, getDownloadURL } from "firebase/storage";
import { useNavigate } from "react-router-dom";
import { toast } from "react-toastify";
import { useAuth } from '../auth/AuthApp/AuthContext';

/**
 * Componente de perfil de usuario
 * Permite visualizar y editar información del perfil del usuario autenticado
 */
function MyProfile() {
  ...
  const handleUpdateProfile = async (e) => {
    ...
    try {
      ...
      // Si hay un nuevo archivo de foto, debería subirlo aquí
      // Esta funcionalidad está incompleta en el código original
      let photoURL = userData.photo;
      
      if (newPhotoFile) {
        // Implementamos la subida de imagen que faltaba
      ...
      // Verificar si existe el documento primero
        ...
        <div className="md:flex">
          ...
          <div className="md:w-1/3 bg-gradient-to-br from-purple-900 to-gray-900 p-6 flex flex-col items-center justify-center">
            <div className="w-40 h-40 rounded-full overflow-hidden mb-4 border-4 border-purple-500 shadow-lg">
              <img 
                src={photoPreview || userData.photo || 'https://i.pravatar.cc/300?u=' + user?.uid} 
                alt="Foto de perfil"
                className="w-full h-full object-cover" 
              />
            </div>
            <h3 className="text-xl font-bold text-white mb-1">
              {`${userData.firstName} ${userData.lastName}`.trim() || "Usuario DJ Nova"}
            </h3>
            ...
            <div className="mb-6">
              ...
              {isEditing ? (
                ...
                <form onSubmit={handleUpdateProfile} className="space-y-4">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-400 mb-1">Foto de perfil</label>
                ...
                <div className="space-y-4">
                  <div className="grid md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <p className="text-white">{userData.firstName || "No especificado"}</p>
                    </div>
                    
                    <div>
                      <h4 className="text-gray-400 text-sm">Apellido</h4>
                      <p className="text-white">{userData.lastName || "No especificado"}</p>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <p className="text-white text-sm font-mono bg-gray-700 px-2 py-1 rounded">{user?.uid}</p>
                  </div>
                </div>
              )}
            </div>
