...
const PlaylistCard = ({ playlist, userName }) => {
  ...
  return (
    <div
      className="bg-gray-900 rounded-xl overflow-hidden hover:bg-slate-700 transition-all group p-2.5 sm:p-3 cursor-pointer"
...
const SkeletonGrid = () => (
  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-4 xl:grid-cols-6 gap-2 sm:gap-3">
    {[...Array(8)].map((_, i) => (
      <div key={i} className="bg-gray-700 rounded-xl p-2.5 animate-pulse">
        <div className="aspect-square bg-gray-600 rounded-lg mb-2"></div>
        <div className="h-2.5 bg-gray-600 rounded mb-1"></div>
...
const OtherUsersPlaylists = () => {
  ...
  useEffect(() => {
    const fetchOtherUsersPlaylists = async () => {
      try {
        ...
        usersSnapshot.forEach((doc) => {
          ...
          if (userId !== user?.uid && userData.playlists && Array.isArray(userData.playlists)) {
            const userName =
        ...
        allPlaylists.sort((a, b) => new Date(b.savedAt || 0) - new Date(a.savedAt || 0));
        setPlaylists(allPlaylists);
      } catch (err) {
        console.error("Error obteniendo playlists de otros usuarios:", err);
    ...
    if (user) fetchOtherUsersPlaylists();
  }, [user]);

  const Header = ({ count }) => (
    <div className="flex flex-wrap items-center justify-between gap-2 mb-3 sm:mb-4">
      <h2 className="text-base sm:text-lg font-bold text-white">Playlists de otros usuarios</h2>
      {count != null && (
  ...
  if (error) {
    return (
      <div className="bg-gray-800 rounded-xl p-4 sm:p-5">
        <Header />
        <div className="bg-red-900/20 border border-red-500/60 rounded-lg p-3 text-red-100 text-sm">
          {error}
        </div>
      </div>
    );
    ...
    return (
      <div className="bg-gray-800 rounded-xl p-4 sm:p-5">
        ...
        <div className="text-center py-8">
          <svg className="w-12 h-12 sm:w-16 sm:h-16 mx-auto mb-3 text-gray-600" fill="currentColor" viewBox="0 0 20 20">
            <path d="M18 3a1 1 0 00-1.196-.98l-10 2A1 1 0 006 5v9.114A4.369 4.369 0 005 14c-1.657 0-3 .895-3 2s1.343 2 3 2 3-.895 3-2V7.82l8-1.6v5.894A4.37 4.37 0 0015 12c-1.657 0-3 .895-3 2s1.343 2 3 2 3-.895 3-2V3z" />
          </svg>
          <p className="text-gray-400 text-sm">No hay playlists de otros usuarios disponibles</p>
        </div>
      </div>
    );
  }
